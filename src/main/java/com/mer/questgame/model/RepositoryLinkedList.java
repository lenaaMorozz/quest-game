...
public class RepositoryLinkedList implements Repository {
    ...
    @Override
    public QuestionTreeNode getQuestionTreeNode() {
        if (questions.size() == 0) {
            fillQuestions();
        }
...

    {
        ...
        context.add("Головокружение и слабость были следствием обезвоживания, последние силы ты потратил на " +
        ...
        context.add("Ты срываешься с высокой пальмы, падаешь с большой высоты на землю, и огромный кокос разбивает тебе голову.");
        answers.add(emptyAnswer);
        answers.add(emptyAnswer);
        ...
        context.add("Ты нашел пальму с бананами, и вот же везение! Рядом с пальмой было много упавших бананов, " +
        ...
        context.add("Ты лег спать и умер от переохлаждения.");
        answers.add(emptyAnswer);
        answers.add(emptyAnswer);
        context.add("Ты нашел чем утеплиться и смог пережить эту холодную ночь! Но ты очень голоден, оглядевшись," +
